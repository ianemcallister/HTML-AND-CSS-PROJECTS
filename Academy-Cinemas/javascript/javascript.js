...
$(document).on("scroll", function() {
    ...
    if ($(document). scrollTop() > 50) {
        // Once the 50px requirement has been met and the
        // nav-shrink class selector to the same HTML element
        // that has the nav class
        $('nav').addClass("nav-shrink");
        // Adjust to position of the mobile drop menu
        // to accomodate the new height diecrease
        $("div.navbar-collapse").css('margin-top', '-6px');
    } else {
        // If the webpage has not been scrolled down or is back at the top of the nav-shrink
        // class selector is removed from the HTML element with the nave class selector
